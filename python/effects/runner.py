...
class RunnerReactive(Effect):
    ...
    def visualize(self, board, y):
        ...
        output = np.array([
            [(((math.sin(i * board.effectConfig["RunnerReactive"]["times"] + (
                        self.position / board.effectConfig["RunnerReactive"]["divide"])) +
                board.effectConfig["RunnerReactive"]["add"]) * ((math.cos(i))))) for i in
        ...
        outputGradient = np.array(
            [
                board.visualizer.multicolor_modes[board.effectConfig["RunnerReactive"]["color_mode"]][0][
                :board.config["N_PIXELS"]],
                board.visualizer.multicolor_modes[board.effectConfig["RunnerReactive"]["color_mode"]][1][
                :board.config["N_PIXELS"]],
                board.visualizer.multicolor_modes[board.effectConfig["RunnerReactive"]["color_mode"]][2][
                :board.config["N_PIXELS"]],
            ]
        )
